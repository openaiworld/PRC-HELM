...
- **`.helm/` 隔离目录**：长期治理产物可统一归档到 `.helm/`，避免污染项目源码目录。
...
├── references/
│   ├── templates.md                 # 需求、技术选型、RFC、ADR、PR、复盘、.helm 状态模板
│   ├── health-scoring.md            # 迭代健康度评分规则
│   ├── rmst-quality.md              # RMST 质量四柱与门禁
│   ├── helm-loop.md                 # .helm 状态目录与多 Agent Loop 协议
...
```text
使用 $prc-helm 为当前项目初始化 .helm 状态目录，包含 HELM.md、agents.md、config.json、state/progress.md 和 state/queue.json。
```
...
Agent 应输出本轮完成情况、未解决风险、下一阶段主要矛盾、迭代健康度评分，以及建议更新的文档，如 `progress.md`、`CHANGELOG.md`、轻量模式的 `docs/retrospective/YYYY-MM-DD.md` 或严格模式的 `.helm/retrospective/YYYY-MM-DD.md`。
...
| `references/templates.md` | 常用治理模板：需求、技术选型、spike、RFC、ADR、PR、复盘、`.helm` 状态等 |
| `references/health-scoring.md` | 迭代健康度评分规则与报告格式 |
| `references/rmst-quality.md` | RMST 可靠性、可维护性、可扩展性、可溯源性门禁 |
| `references/helm-loop.md` | `.helm` 状态目录、多 Agent Loop、队列与终止条件 |
