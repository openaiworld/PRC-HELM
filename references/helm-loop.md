...
本文件描述 PRC-HELM 的严格模式 / Loop 模式。仅当用户要求多 Agent 协同、自动迭代、严格治理、项目长期运行，或目标项目已经采用 `.ai_helm/` 目录时加载。轻量任务不要强制启用 Loop。
...
- 项目根目录已存在 `.ai_helm/HELM.md`、`.ai_helm/state/progress.md` 或 `.ai_helm/state/queue.json`。
...
PRC-HELM 产物优先归档到 `.ai_helm/`，避免污染项目原生源码与业务文档结构。唯一常见例外是项目根目录的 `CHANGELOG.md`，用于发版记录。
...
```text
Step 1: 读取 .ai_helm/HELM.md                # 项目级 PRC-HELM 宪法，如存在
Step 2: 读取 .ai_helm/agents.md              # Agent 角色契约，如存在
Step 3: 读取 .ai_helm/state/progress.md      # 当前主要矛盾、Sprint 上下文
Step 4: 读取 .ai_helm/state/queue.json       # 待执行任务队列
Step 5: 读取 .ai_helm/config.json            # Loop、重试、质量门禁参数
```

若某文件不存在，不要臆造当前状态；应建议创建对应模板，或在任务输出中明确“未启用 .ai_helm 状态目录”。
...
OrchestratorAgent
  1. 重新评估主要矛盾。
  2. 从 .ai_helm/state/queue.json 取最高优先级可执行任务。
